...
def xy_plot(datasets, type=None, fit = None, label_variable = None, aspect = 1.0, yerror = None, x_label = 'X Axis', y_label = 'Y Axis', title = None, box = False, save = False, target_dir = 'output/'):
    if type == '3d':
        ...
        plt.hist(datasets[0], density = False, bins = datasets[1], label = 'Histogram')
        ...
        for i, data in enumerate(datasets):
            if fit is None:
                ...
                plt.scatter(data[0], data[1], color = 'black', linestyle = '--', label = 'Data')
                plt.plot(data[0], fit[i], color = 'red', label = 'Fit')
    ...
    if save:
        ...
        plt.savefig(target_dir+str(title.replace(" ", "_"))+"_plot.png", bbox_inches='tight',pad_inches=0.0, format = 'png')
        plt.show()
    else:
        plt.show()
...
def peak_finder(datax, data, minfit = False, threshold=None, width=None, height=None, distance=None, prominence=None, plot=True):
    maxima = signal.find_peaks(data, height = height, threshold = threshold, width = width, distance=distance, prominence=prominence)[0]
