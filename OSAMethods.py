...
def find_peak_spacing(osax, osay, window_size, width, height, distance, filename, save, target_dir):
    osay = np.array(osay)
    osayc = osay - min(osay)
    osayc = osay
    ...
    plt.plot(osax, osay, linewidth=0.5, alpha=0.5,
                color='lightgrey', label='spectrum', zorder = 3)
    plt.xlabel('Wavelength (nm)')
    plt.ylabel('Intensity (a.u.)')
    plt.ylim(min(osay), 0)
    plt.legend()
...
def lasing_mode_linewidth(osax, osay, peak_locations, save = False):
    indlocs = []
    ...
    for i,loc in enumerate(peak_locations):
        p0 = [max(osay), 0.001, min(osay)]
        lorentz_coeff = odr_auto_lorentz(osax, osay, loc)
        ...
        linewidth = get_linewidth(lorentz_coeff[1], lorentz_coeff[2])
        plt.plot(fitx, lorentz_fit, label=r'$\delta\omega_{}=$'.format(i+1)+str(round(linewidth*10**-9,2))+' GHz', linewidth=0.8, zorder=3, color='tab:blue')
    ...
    title = 'OSA Spectrum Lasing Modes Linewidth Fit'
